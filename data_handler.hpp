...
public:
    ...
    void process(const request_parser& rp, const config_storage& cfg, const cache_storage& cache, const template_storage& tpl, const blog_storage& blogs, std::string& data);
    
private:
    bool process_static(const request_parser& rp, const cache_storage& cache, const config& cfg, std::string& data);
    bool process_blog(const request_parser& rp, const config& cfg, const template_storage& tpl, const blog_storage& blogs, std::string& data);
